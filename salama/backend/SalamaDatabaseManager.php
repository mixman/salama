...
class SalamaDatabaseManager {
    ...
    public static function parse_dsn($dsn) {
        ...
        switch ($parts['scheme']) {
            ...
            case 'oracle':
            case 'mongo':
                if(!isset($parts['path']) || $parts['path'] == '/') $parts['path'] = null;
                if(isset($parts['path'])) $parts['database'] = substr($parts['path'], 1);
                if(!isset($parts['host'])) throw new Exception('No hostname set in data source name');
                $parts['dsn'] = $parts['scheme'] . ':host='
                              . $parts['host'] . ';dbname='
                              . $parts['database'];
                if(isset($parts['port'])) $parts['dsn'] .= ';port=' . $parts['port'];
                break;
            default:
