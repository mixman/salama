...
class QueryTest extends SalamaTestCase {
    ...
    public function testIdentityAware() {
        $user = User::all();
        if ($user instanceof User) $this->assertTrue(true);
        if ($user->info instanceof UserInfo) $this->assertTrue(true);
        $info = $user->info;
        if ($info instanceof UserInfo) $this->assertTrue(true);
        if ($user instanceof User) $this->assertTrue(true);
        $user = User::all();
        if ($info instanceof UserInfo) $this->assertTrue(true);
        if ($user instanceof User) $this->assertTrue(true);
    ...
    public function testIdentityAwareTwo() {
        $res = User::all();
        foreach($res as $k=>$v) {
            if ($res instanceof User) $this->assertTrue(true);
            $this->assertType('User', $v);           
            foreach($v->comments as $k2=>$v2) {
                if ($res instanceof User) $this->assertTrue(true);
                if ($v2 instanceof UserComment) $this->assertTrue(true);
            }
        }
    }
